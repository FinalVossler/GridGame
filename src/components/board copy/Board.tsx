  ...
  const handleClickGrid =
    ...
    () => {
      ...
      if (game.characterInAction) {
        ...
        const clickedEnemy = (
          game.characterInAction.species === SpeciesEnum.A ? teamB : teamA
        ).find(
          (enemy) =>
        ...
        if (
          ...
          if (isDead) {
            (clickedEnemy?.species === SpeciesEnum.B ? setTeamB : setTeamA)(
              (clickedEnemy?.species === SpeciesEnum.B ? teamB : teamA).filter(
                (character) =>
            ...
            (clickedEnemy?.species === SpeciesEnum.B ? setTeamB : setTeamA)(
              (clickedEnemy?.species === SpeciesEnum.B ? teamB : teamA).map(
                (character) => ({
                  ...character,
                  health:
        ...
        // If it's the clicked character's team turn, and the character didn't act yet, and we clicked an accessible grid, then we are about to move to the grid
          ...
          if (
            !(
              game.whichSpeciesTurn === SpeciesEnum.A ? newTeamA : newTeamB
            ).find((el) => !el.actedInTurn)
          ) {
            dispatch(
              ...
              <div
                className={
                  game.characterInAction?.position.column ===
                    characterInPosition.position.column &&
                  game.characterInAction.position.row ===
                    characterInPosition.position.row
                    ? characterInPosition.species === SpeciesEnum.B
                      ? styles.teamBInAction
                      : styles.teamAInAction
                    : characterInPosition.species === SpeciesEnum.B
                    ? characterInPosition.actedInTurn
                      ? styles.actedTeamBMember
                      : styles.teamBMember
                    : characterInPosition.actedInTurn
                    ? styles.actedTeamAMember
                    : styles.teamBMember
                }
              ></div>
            )}
